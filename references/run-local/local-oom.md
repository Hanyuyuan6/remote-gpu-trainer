...
The mechanics and the fit-it ladder are `references/training/oom-memory.md`; this file holds the local-only
residue (you share the box with a desktop/IDE) and the rule about where heavy DL belongs.

## Which OOM is it

### LO1 — VRAM OOM is not host-RAM OOM (and locally, your desktop is also eating RAM)

Which OOM you have (`torch.OutOfMemoryError` traceback ⇒ VRAM; bare `Killed` / exit 137 ⇒ host-RAM) and the
cost-ordered ladder for each → `references/training/oom-memory.md` M3, M4.

The local residue: on a workstation you also use interactively, the host-RAM headroom is smaller than you
think because the browser, IDE, and desktop environment already hold several GB, so the `Killed` case is
easier to misread here than on a rental. Confirm which one before changing anything — a host-RAM kill is
fixed by lowering `num_workers`, by not materializing the dataset in every worker (lazy / memory-mapped
loading), and by closing the other RAM consumers (LO3), *not* by a smaller batch.

## The local fit-it path

### LO2 — The fit-it ladder lives in oom-memory.md

The cost-ordered rungs, their exact flags, and the per-rung reasoning → `references/training/oom-memory.md`
M4; every "changes the science" rung (seq-len / resolution change, LoRA capacity change) is a re-verify
trigger (`references/verifying/methodology.md`). The one local-only fact: on a **single** local card the
FSDP / ZeRO sharding rung (M4 rung 8) is unavailable, so failing to fit through the single-card rungs
(1–7, 9, 10) is the signal to move the job to a bigger / multi-GPU rental, not to keep forcing the local box.

### LO3 — Free the local GPU/RAM before blaming the model (close the other consumers)
...
free -h                                 # close browser/IDE if host RAM is the squeeze (LO1 host-RAM case)
```
...
conclude the *model* doesn't fit and climb the LO2 ladder. (Zombie-VRAM detail →
`references/run-remote/gotchas_universal.md` U11.)

## The rule

### LO4 — Heavy DL goes on the GPU; the local dev box is for STATIC checks only
