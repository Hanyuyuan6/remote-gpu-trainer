...
The per-platform substrate for RunPod Pods — **the Docker image IS the env contract**, a three-tier storage model where the durable mount differs from the parking one, and a teardown verb (`terminate`) that DELETES the volume disk. Universal gotchas are NOT repeated — see `references/run-remote/gotchas_universal.md`.
...
**Mental-model shift vs AutoDL (the one fact that breaks portability):** AutoDL persists `/root` across a power-off so "关机 to save money, restart later" is safe, whereas a stopped RunPod Pod is pinned to one physical machine whose GPU can be rented away (RP1) and still bills the volume disk at 2× (RP4).
...
- **REST API** — the current first-class automation surface: `POST /v2/pods` (and `/pods/{id}/start|stop`, `DELETE`). The create body takes `cloudType: SECURE|COMMUNITY` and **`interruptible: true|false`** for Spot (verified docs.runpod.io/api-reference/pods/POST/pods 2026-06). The newer REST create-Pod input has **no `bidPerGpu` field**; interruptible is a plain boolean. The legacy **GraphQL** `podRentInterruptable`/`bidPerGpu` bid mutation still exists for the old API surface — if a script sets a bid, it is on the GraphQL path, not REST.
...
- **Egress / proxy / China mirror: N/A.** Free egress, regions across NA + Europe + Oceania + Asia-Pacific (e.g. `AP-IN-1` India added 2026-04), **no mainland-China datacenter** (verified runpod.io/blog/new-runpod-datacenter-now-live-ap-in-1 2026-06). No `/etc/network_turbo` equivalent and no China mirror needed; `pip`/`hf`/`apt` reach the open internet directly. For HF big-shard stalls the fix is **not** a mirror — `pip install huggingface_hub[hf_transfer]` + `export HF_HUB_ENABLE_HF_TRANSFER=1`, and point `HF_HOME` at the Network Volume so re-downloads survive Pod churn (RP11 below; transport verbs → `huggingface-skills:hf-cli`).
...
- **Public IP stability differs by cloud:** Community Cloud public IPs **may change on migration/restart**; Secure Cloud IPs "should remain stable" (verified expose-ports 2026-06). A pinned SSH target is safer on Secure Cloud.
...
- **RP14 — a successful delete response is not proof the Pod is gone; poll until it 404s.** Symptom: teardown "succeeded" (the delete call returned OK) but the Pod is still listed, so teardown/billing status is not yet proven. Root cause: the control plane acknowledging a delete is not the same as the resource being destroyed — a stale show can still return it. Fix: after the destroy call (e.g. `DELETE /v1/pods/{id}` — verify against current docs), poll the show endpoint (e.g. `GET /v1/pods/{id}`) until it returns 404 before declaring teardown complete; in our run a singular `/v2/pod/{id}/terminate`-style path did not destroy the Pod at all. (verified 2026-06, observed on live multi-provider runs)
- **RP15 — "No spot capacity" can be account/GPU-type-gated, not market-dry.** Symptom: a GPU type shows no interruptible/Spot availability even when the market isn't obviously exhausted. Root cause: lower-priority (interruptible) capacity must be enabled for the account and that GPU type before it is offered — the empty result can be a config gate, not market scarcity. Fix: before concluding "no spot available," confirm interruptible capacity is enabled for the account and that specific GPU type; "no spot" can mean "not enabled here," not only a dry market. (verified 2026-04, observed on live multi-provider provisioning)
